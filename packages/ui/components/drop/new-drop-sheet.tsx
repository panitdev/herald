...
export function NewDropSheet({ open, mode: initialMode, onOpenChange, onSave }: Props) {
  const { t } = useTranslation()
  ...
  // Reset state and sync tab when sheet opens
  React.useEffect(() => {
    if (open) {
      setTab(initialMode)
      setFileItems([])
  ...
  // Auto-read clipboard when clipboard tab is active
  React.useEffect(() => {
  ...
  return (
    <Sheet open={open} onOpenChange={onOpenChange}>
      <SheetContent side="bottom" className="h-[85dvh] rounded-t-xl px-0 pb-0">
        <SheetHeader className="border-b border-border px-6 pb-4">
          <SheetTitle>{t("drop.new.title")}</SheetTitle>
          <SheetDescription className="sr-only">{t("drop.new.description")}</SheetDescription>
        </SheetHeader>
